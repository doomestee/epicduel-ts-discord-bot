...
export default new EDEvent("onComparisonUpdate", async function (hydra, { part, type }) {
    if (type === 0) this.swarm.resources.checkpoints.comparison[0] = 1;
        ...
        //  */
        // let compare = (oldObj, newObj, identifier) => {//(compType="item") => {
        //     /**
        ...
        //      */
        //     let ids = {
        //         changed: [], removed: [], added: [],//oldObj.map(v => [v[identifier], v]), added: []
        //     };
        ...
        // client.zamn = {
        //     item: compare(client.comparisonFiles.item, epicduel.client.boxes.item.objMap.toArray(), "itemId"),
        //     skills: {
        ...
        // // cri
        // if (client.zamn.item[0] === 1) {
        //     hasUpdated = true;

        //     let adds = []; let removes = []; let changes = [];

        //     for (let x = 0; x < client.zamn.item[1]["+"]?.length; x++) {
        //         adds.push(client.zamn.item[1]["+"][x]);
        //     }

        //     for (let x = 0; x < client.zamn.item[1]["-"]?.length; x++) {
        //         removes.push(client.zamn.item[1]["-"][x]);
        //     }

        //     for (let x = 0; x < client.zamn.item[1]["~"]?.length; x++) {
        //         changes.push(client.zamn.item[1]["~"][x]);
        //     }
        ...
        //      */
        //     let embed = (type, item) => {
        //         let obj = (type === "~") ? client.comparisonFiles.item.find(v => v.itemId === item[0]) : item[1];
        ...
        //         await client.rest.guilds.getActiveThreads("565155762335383581")
        //             .then((v) => {
        //                 if (v.threads.length) {
        ...
        //                         if (thread.parentID !== "1159896720717656205") continue;

        //                         if (thread.name.includes(epicduel.client.currVersion)) {
        ...
        //         if (threads.a !== "") threadies.push(client.rest.channels.getMessages(threads.a, { after: threads.a.slice(0, -2) + "0", limit: 1 }));
        //         if (threads.c !== "") threadies.push(client.rest.channels.getMessages(threads.c, { after: threads.c.slice(0, -2) + "0", limit: 1 }));
        //         if (threads.r !== "") threadies.push(client.rest.channels.getMessages(threads.r, { after: threads.r.slice(0, -2) + "0", limit: 1 }));

        //         if (adds.length    && threads.a === "") threads.a = await client.rest.channels.startThreadInForum("1159896720717656205", { message: { content: adds.length + " item(s) were added, as of v" + epicduel.client.currVersion   }, name: "New Items - v" + epicduel.client.currVersion }).then(v =>     v.edit({appliedTags: ["1159899392287981578"]})).then(v => v.id);
        //         if (changes.length && threads.c === "") threads.c = await client.rest.channels.startThreadInForum("1159896720717656205", { message: { content: changes.length + " item(s) were changed, as of v" + epicduel.client.currVersion }, name: "Changed Items - v" + epicduel.client.currVersion }).then(v => v.edit({appliedTags: ["1159899426542854255"]})).then(v => v.id);
        //         if (removes.length && threads.r === "") threads.r = await client.rest.channels.startThreadInForum("1159896720717656205", { message: { content: removes.length + " item(s) were removed, as of v" + epicduel.client.currVersion }, name: "Removed Items - v" + epicduel.client.currVersion }).then(v => v.edit({appliedTags: ["1159899451779977316"]})).then(v => v.id);
        //     }
        ...
        //         await client.rest.channels.createMessage(threads.a, {
        //             embeds: embeds
        //         }).then(m => { if (firsts.a === undefined) firsts.a = m.jumpLink; }).catch((err) => { logger.error(err); });
        //     }
        ...
        //         await client.rest.channels.createMessage(threads.r, {
        //             embeds: embeds
        //         }).then(m => { if (firsts.r === undefined) firsts.r = m.jumpLink; }).catch((err) => { logger.error(err); });
        //     }
        ...
        //         await client.rest.channels.createMessage(threads.c, {
        //             embeds: embeds
        //         }).then(m => { if (firsts.c === undefined) firsts.c = m.jumpLink; }).catch((err) => { logger.error(err); });
        //     }
        ...
        //     /*
        //     for (let i = 0; i < client.zamn.item[1]["+"].length; i++) {
        //         let item = client.zamn.item[1]["+"][i];
        ...
        // if (false && Object.keys(client.zamn.skills).some(v => client.zamn.skills[v][0] === 1)) {
        //     //hasUpdated = true;

        //     let adds = []; let removes = []; let changes = [];

        //     for (let x = 0; x < Object.keys(client.zamn.skills).length; x++) {
        //         let key = Object.keys(client.zamn.skills)[x];
        //         if (key === "passiveStatRules") continue; // This one is a big gigantic mess, idk how or why is this...
        //         if (client.zamn.skills[key][0] !== 1) continue;

        //         hasUpdated = true;

        //         for (let y = 0; y < Object.keys(client.zamn.skills[key][1]).length; y++) {
        //             let key2 = Object.keys(client.zamn.skills[key][1])[y];

        //             for (let z = 0; z < client.zamn.skills[key][1][key2].length; z++) {
        //                 let skill = client.zamn.skills[key][1][key2][z];
        ...
        //         let obj = (type === "~") ? client.comparisonFiles.skills[skill["type"]].find(v => v[client.zamn.skills[skill["type"]]['_id']] === skill[0]) : skill[1];
        //         let jsonStringified = JSON.stringify(obj, 0, 2);
        ...
        // if (hasUpdated) {
        //     client.comparisonFiles = {
        //         // TODO: skilllllllllllls objlist cri
        //         skills: epicduel.client.boxes.skills.objList,
        //         item: epicduel.client.boxes.item.objMap.toArray()
        //     };

        //     client.rest.channels.createMessage("1034498187911774278", {
        //         files: [{
        //             contents: JSON.stringify({skills: epicduel.client.boxes.skills.objList, item: epicduel.client.boxes.item.objMap.toArray()}),
        //             name: "jason.json"
        //         }]
        //     }).catch((err) => logger.error(err));
        // }
