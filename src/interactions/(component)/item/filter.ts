...
export default new Command(CommandType.Component, { custom_id: "item_cat_<type>_<userId>" })
    .attach('run', async ({ client, interaction, variables }) => {
        ...
        let options = interaction.data.values.raw as OptType[];
        ...
        for (let x = 0; x < (10) && x < rewards.length; x++) {
        ...
        // list_item_<type>_<0>_<userId>

        const components:MessageActionRow[] = [{
            type: 1, components: [{
                type: ComponentTypes.BUTTON, customID: "list_item_0_" + ((0) - 1) + "_" + (userId === "000" ? interaction.user.id : userId),
                style: ButtonStyles.PRIMARY, label: "<", disabled: ((0) < 1)
            }, {
                type: ComponentTypes.BUTTON, customID: "promptidk",
                style: ButtonStyles.SECONDARY, label: "...", disabled: true,
            }, {
                type: ComponentTypes.BUTTON, customID: "list_item_0_" + ((0) + 1) + "_" + (userId === "000" ? interaction.user.id : userId),
                style: ButtonStyles.PRIMARY, label: ">", disabled: ((0) >= (Math.ceil(rewards.length / 10) - 1))
            }]
        }, {
            type: 1, components: [{ // list_item_<type>_<0>_<userId>
