import { ActionRowBase, ButtonComponent, ButtonStyles, ComponentTypes, MessageActionRow, SelectMenuComponent, SelectOption, StringSelectMenuOptions, TextButton } from "oceanic.js";
...
import { getHighestTime, getUserLevelByExp } from "../../util/Misc.js";
...
function find<T extends any>(arr: Array<T>, pred: (val: T, index: number, obj: T[]) => boolean) {
...
export default new Command(CommandType.Application, { cmd: ["leaderboard", "fetch"], description: "Fetches the leaderboard!", waitFor: ["EPICDUEL", "LOBBY"], cooldown: 3000, gateVerifiedChar: 1 })
    .attach('run', async ({ client, interaction }) => {
        ...
        const table = new AsciiTable3("test").setAlignLeft(1).setStyle("unicode-double").setHeading("No.", "Name", ...headings(type))
        ...
        let components = [{
            ...
            components: [{
                type: 3, customID: "select_lb_" + (isFaction ? "2" : "1") + "_000", placeholder: "Select to see their partial info (cached)!",
        ...
        const volues = await DatabaseManager.cli.query<IFaction | ICharacter>(`SELECT * FROM ${isFaction ? "faction" : "character"} WHERE name IN (${quickDollars(leaders.length)})`, leaders.map(v => v.name))
            .then(v => v.rows);
        ...
        for (let i = 0; i < mergedArr.length; i++) {
            ...
            (components[0].components[0] as StringSelectMenuOptions).options.push({
                label: item.name,
                ...
                value: item.available ? "a" + item.id : "u" + item.name
            });
        }

        let files:import("oceanic.js").File[] = [];
